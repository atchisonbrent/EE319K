				...
				if(Characters[j].active)											// check if 2nd character is active
					if(i != j){																	// Can't collide with yourself
						...
						if(temp){
							...
							if(Characters[i].potato == 1 && Characters[j].potato == 0){
								HotPotato_SetIt(j);
							}
						}		
					}	
	...
	for(int i = 0; i < characterCount ; i++){												// update all player-collisions
		...
		if(Characters[i].active){													// check if 1st character is active
			if(Characters[i].x <= 0){
				...
				if(Characters[i].y >= (Map[currentMap].height*mapScale))
					Characters[i].cStateS=1;												// map bottom-left border collision
			}
			else if(Characters[i].x + Characters[i].width >= (Map[currentMap].width*mapScale)){
			...
			else if(Characters[i].y >= (Map[currentMap].height*mapScale))
